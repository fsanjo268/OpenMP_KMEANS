...
void initCentroids(const float *data, float* centroids, int* centroidPos, int samples, int K)
{
	int i;
	int idx;
    #pragma omp parallel for shared(centroidPos, centroids, data, samples), private(i), schedule(dynamic)
	for(i=0; i<K; i++)
	{
		idx = centroidPos[i];
...
int classifyPoints(float *data, float *centroids, int *classMap, int lines, int samples, int K){
	...
	float dist, minDist;
	int changes=0;
    #pragma omp parallel for private(i,j, class, dist, minDist) shared(data, centroids, classMap, lines) reduction(+:changes)	
	for(i=0; i<lines; i++)
	{
		class=1;
		minDist=FLT_MAX;
...
float recalculateCentroids(float *data, float *centroids, int *classMap, int lines, int samples, int K){
    ...
    #pragma omp parallel for private(i,j) shared(classMap, pointsPerClass, auxCentroids, data, samples) schedule(guided)
	for(i=0; i<lines; i++) {
        class=classMap[i];
    ...
    #pragma omp parallel for private(i, j) shared(auxCentroids, pointsPerClass, samples) schedule(guided)
	for(i=0; i<K; i++) 
	{
		for(j=0; j<samples; j++){
            #pragma omp atomic update
    ...
    #pragma omp parallel for private(i) shared(distCentroids, centroids, auxCentroids, samples) reduction(max:maxDist) schedule(guided)
	for(i=0; i<K; i++){
